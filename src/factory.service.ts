...
export class FactoryService implements OnModuleInit, OnModuleDestroy {
  ...
  use<T>(factoryClass: FactoryClass<T>): FactoryBuilder<T> {
    ...
    return new FactoryBuilder<T>(instance as BaseFactory<T>, this._faker, this.dataSource ?? null);
  }
  ...
  async seed(): Promise<void> {
    ...
    this.emit(FACTORY_EVENTS.SEED_ALL_COMPLETED, { seederCount: seeders.length });
  }
